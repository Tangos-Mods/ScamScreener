...
/**
 * Rule tuning screen for the centralized detection settings.
 *
 * <p>The full rule set no longer fits on a single screen, so the controls are
 * split into sections that can be paged through.
 */
public final class RulesSettingsScreen extends BaseScreen {
    ...
    /**
     * Creates the rules settings screen.
     *
    ...
    /**
     * Builds the current section controls.
     */
    @Override
    protected void init() {
    ...
    /**
     * Draws the active section header and small context notes.
     *
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        y += 12;
        drawLine(context, left, y, "Changes save immediately and rebuild the pipeline.");
        y += 12;
    ...
    private void buildBehaviorActions(List<SettingAction> actions) {
        ...
        addAction(actions, "Combo Min: " + behaviorStage.getComboBonusMinimum(),
            () -> behaviorStage.setComboBonusMinimum(nextCounter(behaviorStage.getComboBonusMinimum(), 1, MAX_THRESHOLD)));
